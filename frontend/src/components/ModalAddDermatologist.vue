<template>
  ...
  <div>
    <b-button
      @click="onshow(selected)"
      variant="success"
      v-if="type == 'PHARMACY_ADMINISTRATOR'"
      >Add</b-button
    >
    <b-modal v-model="show" ref="modal" title="Add dermatologist" hide-footer>
      ...
      <br />
      <b-button variant="success" @click="createNewWorkingDays"
        >CREATE</b-button
      >
    </b-modal>
  </div>
...
export default {
  ...
  methods: {
    ...
    removeNew(item) {
      this.workingDays.splice(this.workingDays.indexOf(item), 1);
    },
