            ...
            <thead class="thead-inverse">
                ...
                <tbody>
                    <tr v-for="comp in listComp" :key="comp.id">
                        ...
                        <td><a name="" id="" class="btn btn-primary" v-bind:href="'/OneComplaint/'+comp.id" role="button">Detailed</a></td>
                    </tr>
                </tbody>
        </table>
        </div>
...
export default {
    ...
    mounted() {
        axios.get("http://localhost:9005/api/user/getComplaint/"+this.$store.getters.getUser.id)
            .then(res => {
